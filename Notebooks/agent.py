...
class SuperAgentHybridResponsesAgent(ResponsesAgent):
    ...
    def __init__(self, workflow: StateGraph):
        ...
        # Embedding configuration for DatabricksStore (long-term memory)
        self.embedding_endpoint = config_dict.get('lakebase_embedding_endpoint', 'databricks-gte-large-en')
        self.embedding_dims = config_dict.get('lakebase_embedding_dims', 1024)
        
        # Lazy-initialized memory components
        self._store = None
    ...
    def store(self):
        ...
        if self._store is None:
            logger.info(f"Initializing DatabricksStore with instance: {self.lakebase_instance}")
    ...
    def predict_stream(
        ...
        """
        # Get identifiers using helper methods
        ...
        # Track workflow timing (TTFT and TTCL)
...
- Return results with proper context and explanations"""),
        ...
        with CheckpointSaver(instance_name=self.lakebase_instance) as checkpointer:
            ...
            # Stream modes: updates, messages, custom, tasks
            for event in app.stream(initial_state, run_config, stream_mode=["updates", "messages", "custom", "tasks"]):
                ...
                # Handle node updates (updates mode)
                    ...
                    if first_message:
                        ...
                        # Emit node name as a step indicator
                        if events:
...
# Create the Hybrid Super Agent workflow from modular code
...
print("="*80)
print("Architecture: Modular Agents + Explicit State Management")
print("Benefits:")
print("  ✓ Modular and testable agent classes (from src/multi_agent/)")
...
print("  ✓ SQL validation and execution tracking")
